...
export interface ComingSoonProps {
  ...
  /** splash 전용, 기본 "Coming soon!" — 존재하지 않는 예약처럼 다른 문구가 필요한 화면에서만 넘긴다. */
  title?: ReactNode;
  /** splash 전용, 기본 "/coming-soon-bg.jpg" — 화면마다 다른 배경 사진을 쓰고 싶을 때(각 앱 public/에 파일 필요). */
  image?: string;
}

export function ComingSoon({ label, variant = "default", title, image }: ComingSoonProps) {
  if (variant === "splash") {
    return (
      <div className="relative flex min-h-[70vh] items-center justify-center overflow-hidden bg-[#131b2e]">
        {/* 배경 사진 — 기본은 각 앱 public/coming-soon-bg.jpg(logo-mb.png와 같은 관례: 공용
            컴포넌트는 경로만 참조하고, 실제 파일은 이 컴포넌트를 쓰는 앱마다 자기 public에
            둔다). image를 넘기면 그 경로를 쓴다(예: 존재하지 않는 예약 전용 일러스트). */}
        <img
          src={image ?? "/coming-soon-bg.jpg"}
          alt=""
          aria-hidden="true"
