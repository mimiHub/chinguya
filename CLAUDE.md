...
작업 진행 상황은 노션 DB 「페이지 정의서」로 공유한다(민철·퍼블리셔 공용).

- DB: https://app.notion.com/p/e4b42d23f25f4109b9c3736ad05ace21
- data source: `collection://725b7fdc-694c-4b20-92d6-fdae7154c95b`
- 식별 키: `화면코드`. 화면코드·화면명의 원천은 이 저장소 안의 와이어프레임
  `docs/*.md` 화면 ID 매핑표다.
...
|---|---|
| 사이트 | 고객 / 관리자 / 여행사 |
| 진행 단계 (select) | 기획완료 → 퍼블완료 → 개발완료 |
| 재작업 (multi-select) | 기획수정 / 퍼블수정 |
| API 상태 (select) | 진행전 → mock완료 → api완료 |

`API 상태`는 **백엔드(`chinguya-api`) 소관**이다. 이 저장소에서는 읽기만 한다.

### 갱신 트리거

Claude는 아래 상황에서 **요청받지 않아도** 해당 행을 갱신하고, 무엇을
바꿨는지 보고한다.

- 새 와이어프레임이 추가됨 → 행 추가 + `진행 단계 = 기획완료`
- 퍼블 결과물 commit·push 요청 → `진행 단계 = 퍼블완료`
- MSW 목 핸들러 연결 → `API 상태 = mock완료`
- 실제 API 연결 완료 후 commit·push 요청 → `진행 단계 = 개발완료`
  (`API 상태 = api완료`는 백엔드가 찍는다. 붙이려는 화면이 아직 `api완료`가
  아니면 갱신하지 말고 사람에게 알린다.)

`API 상태`는 웹 입장에서 **작업 가능 신호**다. `mock완료 → api완료` 전환이
곧 "이제 목을 떼고 실제 API에 붙여도 된다"는 뜻이다.

### 재작업 태그

- 퍼블·개발이 진행 중이거나 끝난 화면의 **와이어프레임**이 바뀌면
  `기획수정` 부착
- 개발이 진행 중이거나 끝난 화면의 **퍼블**이 바뀌면 `퍼블수정` 부착
- 후속 반영을 마친 쪽이 **직접 해제**한다. 퍼블이 기획 변경을 반영했으면
  `기획수정`을, 개발이 퍼블 변경을 반영했으면 `퍼블수정`을 뗀다.
- 남아 있는 태그 = 아직 따라잡지 못한 작업. 이것이 리스크 목록이다.

### 진행 단계는 전진만

한 번 올라간 `진행 단계`는 되돌리지 않는다. 퍼블이 수정돼도 `개발완료`를
`퍼블완료`로 낮추지 않고 `퍼블수정` 태그로만 표현한다. 그래야 "어디까지
갔는지"와 "무엇이 밀렸는지"가 한 줄에서 동시에 보인다.

### 갱신 절차

1. 갱신 전 `화면코드`로 해당 행을 조회한다.
2. 행이 있으면 **갱신**, 없을 때만 추가한다. (두 사람이 동시에 작업하므로
   조회 없이 추가하면 같은 화면이 두 줄 생긴다.)
3. 이미 목표 값이면 그대로 두고 덮어쓰지 않는다.
4. 갱신 후 바꾼 행 목록을 사람에게 보고한다.

### Claude가 하지 말 것

- 셀렉트 옵션값을 임의로 추가하지 않는다. 위 표의 값만 쓴다.
- 행·컬럼을 삭제하지 않는다. 정리가 필요하면 사람에게 알린다.
- 다른 사람이 방금 바꾼 값을 근거 없이 덮어쓰지 않는다.
- 판단이 애매하면 갱신을 멈추고 묻는다.
