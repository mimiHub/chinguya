 ...
 * 항상 떠 있는 팝업이 화면 절반을 덮으면 본문을 읽을 수 없기 때문. 펼치면 옵션·날짜·수량이 위로 열린다.
 * 시트 본문은 자체 스크롤이고 overscroll을 막지 않아서, 맨 위에서 더 밀면 스크롤이 페이지로 넘어간다.
 *
...
export function BookingDock({ children, footer, defaultOpen = false, popupClassName = "" }: BookingDockProps) {
  const bodyId = useId();
  ...
  const scrollerClass = isMobile
    ? "flex max-h-[calc(80dvh_-_22rem)] flex-col gap-4 overflow-y-auto border-y border-line bg-bg px-6 py-4"
    : "flex flex-col gap-4";
