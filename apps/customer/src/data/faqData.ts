...
/**
 * FAQ 목업(S4-C3). 카테고리·검색 없이 관리자 CMS-lite(S4-A1/A3,
 * apps/admin/src/data/faqData.ts)에서 정한 순서(order) 그대로 보여준다 — 실제로는
 * GET /api/faq 응답으로 대체될 자리다.
 *
 * 관리자 쪽 파일과 같은 시드(id·순서)로 시작하지만, 지금은 두 앱이 독립된 프로토타입이라
 * 관리자에서 편집해도 여기엔 반영되지 않는다(실연동 전까지의 알려진 한계).
 */
