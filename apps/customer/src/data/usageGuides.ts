...
/**
 * "사용방법" 안내 콘텐츠 — 자산 카테고리(자전거/낚싯대)별 단계 목록. 같은 데이터를 두 곳에서 그대로 보여준다:
 *   1) 상품 상세(/rental/[id]) "상품 사용방법" 탭 — 예약하기 전에 볼 수 있다.
 *   2) 고객지원(/contact) "사용방법" 탭 — 예약한 뒤에도(대여 중에도) 언제든 찾아볼 수 있다.
 * 두 화면 모두 UsageGuideSteps 컴포넌트가 이 데이터를 그리므로, 여기만 고치면 양쪽이 같이 바뀐다.
 *
